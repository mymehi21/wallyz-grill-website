...
export function CartProvider({ children }: { children: ReactNode }) {
    ...
    for (const d of activeDiscounts) {
      ...
      if (d.scope === 'store') {
        if (d.type === 'percentage') {
          ...
          // BOGO store-wide: cheapest item free for every 2 items
        ...
        for (const item of affectedItems) {
          ...
          if (d.type === 'percentage') {
            ...
            // BOGO on specific item: every other item free
  ...
  const discountedTotal = Math.max(0, cartTotal - totalSavings);

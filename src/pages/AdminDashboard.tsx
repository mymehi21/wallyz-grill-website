...
import { LogOut, ShoppingBag, Truck, Users, Star, Menu as MenuIcon, X, Printer, Trash2, RotateCcw, Archive, Search, ArrowUpDown, Calendar, UtensilsCrossed, Clock, Save, Plus, MapPin, Tag, TrendingUp } from 'lucide-react';
...
export default function AdminDashboard({ onLogout, adminUser }: AdminDashboardProps) {
  ...
  const fetchAllData = async () => {
    ...
    try {
      const [ordersRes, cateringRes, truckRes, jobsRes, reviewsRes] = await Promise.all([
        supabase.from('pickup_orders').select('*').is('deleted_at', null).eq('location_id', selectedLocationId).in('status', ['paid', 'paid_clover_sync_failed']).order('created_at', { ascending: true }),
        supabase.from('catering_menu_orders').select('*').is('deleted_at', null).eq('location_id', selectedLocationId).order('created_at', { ascending: true }),
        ...
        supabase.from('customer_reviews').select('*').is('deleted_at', null).eq('location_id', selectedLocationId).order('created_at', { ascending: true }),
      ]);
  ...
  const fetchHours = async () => {
    const key = `business_hours_${selectedLocationId}`;
    const { data } = await supabase.from("site_settings").select("*").eq("setting_key", key).maybeSingle();
  ...
  const saveHours = async () => {
    setHoursSaving(true);
    const key = `business_hours_${selectedLocationId}`;
    const { error } = await supabase.from("site_settings").upsert({ setting_key: key, setting_value: JSON.stringify(hours) }, { onConflict: "setting_key" });
    setHoursSaving(false);
    if (!error) { setHoursSaved(true); setTimeout(() => setHoursSaved(false), 3000); }
    else alert("Failed to save hours: " + error.message);
  };
  ...
  const renderCatering = () => (
    <div className="space-y-6">
      ...
      {cateringSubTab === 'party-trays' && (
        <div>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
            <h3 className="text-xl font-bold text-white">Catering Tray Orders</h3>
            <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
              <div className="relative flex-1 sm:flex-initial">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" size={18} />
                <input
          ...
          {renderItems(cateringOrders, 'catering_menu_orders', 'Catering Tray Orders')}
        </div>
      )}
  ...
  const renderHours = () => (
    <div className="max-w-2xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        <h2 className="text-2xl font-bold text-white">Business <span className="text-orange-500">Hours</span></h2>
        <button
          onClick={saveHours}
          disabled={hoursSaving}
          className={`flex items-center gap-2 px-6 py-2 rounded-lg font-semibold transition-colors ${
            hoursSaved ? "bg-green-500 text-white" : "bg-orange-500 hover:bg-orange-600 text-white"
          } disabled:opacity-50`}
        >
          <Save size={18} />
      ...
      <div className="space-y-3">
        {hours.map((h, i) => (
          <div key={h.day} className={`bg-gray-800 rounded-lg p-4 border ${
            h.closed ? "border-red-900 opacity-60" : "border-gray-700"
          } flex flex-wrap items-center gap-4`}>
            <div className="w-28">
              <p className="text-white font-semibold">{h.day}</p>
            </div>
              ...
              <>
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    className="bg-gray-900 border border-gray-700 text-white px-3 py-1.5 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"
                  />
                </div>
                <div className="flex items-center gap-2">
      ...
      <p className="text-gray-500 text-sm mt-4">Changes are saved to the database and will reflect on the Contact page automatically.</p>
    </div>
  );
  return (
        ...
        >
          <div className="p-6 h-full overflow-y-auto flex flex-col">
            ...
            <nav className="space-y-2">
              {/* Location Switcher */}
              <div className="mb-4">
                <p className="text-gray-500 text-xs uppercase font-semibold mb-2 px-4">Location</p>
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${
                  activeTab === "hours" ? "bg-orange-500 text-white" : "text-gray-400 hover:bg-gray-800"
                }`}
              >
                <Clock size={20} />
                Business Hours
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${
                  activeTab === "activity" ? "bg-orange-500 text-white" : "text-gray-400 hover:bg-gray-800"
                }`}
              >
                <TrendingUp size={20} />
                Activity
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${
                  activeTab === "discounts" ? "bg-orange-500 text-white" : "text-gray-400 hover:bg-gray-800"
                }`}
              >
                <Tag size={20} />
                Discounts & Promos
        ...
        <div className={`flex-1 transition-all duration-300 ${sidebarOpen ? 'ml-64' : 'ml-0'}`}>
          <div className="bg-gray-900 border-b border-gray-800 px-6 py-4 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2 bg-orange-500 bg-opacity-20 border border-orange-500 px-3 py-1.5 rounded-lg">

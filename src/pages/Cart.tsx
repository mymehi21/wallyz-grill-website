...
export default function Cart({ onNavigate }: CartProps) {
  const { cart, removeFromCart, updateQuantity, cartTotal, discountedTotal, appliedDiscounts, totalSavings } = useCart();
    ...
    <section id="cart" className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black text-white py-20 pt-28">
      <div className="container mx-auto px-4">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-gray-800 rounded-lg p-6 border border-orange-500 mb-8 space-y-3">
            <div className="flex justify-between items-center text-gray-300">
              <span>Subtotal:</span>
              <span>${cartTotal.toFixed(2)}</span>
            </div>
            {appliedDiscounts.map(d => (
              <div key={d.id} className="flex justify-between items-center text-green-400 text-sm">
                <span>🏷️ {d.name}</span>
                <span>-${d.savings.toFixed(2)}</span>
              </div>
            ))}
            {totalSavings > 0 && <div className="border-t border-gray-700 pt-2" />}
            <div className="flex justify-between items-center text-2xl font-bold">
              <span>Total:</span>
              <span className="text-orange-500">${discountedTotal.toFixed(2)}</span>
            </div>
            {totalSavings > 0 && (
              <p className="text-green-400 text-sm text-center font-semibold">You save ${totalSavings.toFixed(2)}! 🎉</p>
            )}
          </div>

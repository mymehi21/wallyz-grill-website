  ...
  try {
    ...
    if (order.clover_checkout_session_id) {
      ...
      if (sessionRes.ok) {
        ...
        console.log(`Clover session lookup returned ${sessionRes.status}: ${errText}`);

        // 404 means the session expired (Clover expires sessions after 15 min)
        // but the payment already went through — Clover only redirects to success URL
        // after confirmed payment. Trust the redirect.
        if (sessionRes.status === 404) {
          console.log('Session expired but redirect came from Clover success URL — trusting payment');
          paymentVerified = true;
        } else {
          // Other errors (401, 500 etc) — cannot verify
    ...
    try {
      ...
      if (twilioSid && twilioToken && twilioFrom && order.customer_phone) {
        const locationName = order.location_id === 'location1' ? 'Oak Park' : 'Redford';
        const itemsList = cart.map((item: any) => `${item.quantity}x ${item.name}`).join(', ');
        const pickupTime = order.pickup_time || 'ASAP';
        const smsBody = `Hi ${order.customer_name}! Your Wallyz Grill order has been received ✅

Items: ${itemsList}
Total: $${Number(order.total_amount).toFixed(2)}
Pickup: ${pickupTime}
Location: Wallyz Grill - ${locationName}
...
Thank you for your order!`;
      ...
      console.error('SMS failed (non-critical):', smsErr);
    }
